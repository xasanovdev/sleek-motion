...
export const autoHeightVariants: Variants = { visible: ({ height }: { height?: number } = {}) => ({ height: height ?? "auto" }) };
...
export function AutoHeight<T extends AnimationTag = "div">({
  children, duration, delay, reducedMotion, style, ...props
}: AnimationProps<T>) {
  const { ref, height } = useMeasuredHeight();
  const reduce = useMotionPreference(reducedMotion);
  return <MotionSurface {...props} initial={false} animate={{ height: height ?? "auto" }}
    transition={tween(reduce ? 0 : duration, reduce ? 0 : delay)} style={{ ...style, overflow: "clip" }}>
