...
const groupNotes: Record<Category, { avoid: string; accessibility: string; performance: string }> = {
  ...
  Overlays: { avoid: "Do not treat motion wrappers as complete accessible overlays. Keep positioning, outside-click dismissal, focus and keyboard behavior in the consuming UI.", accessibility: "Modal and drawer examples use a native dialog for focus trapping, Escape and focus restoration. Popover and dropdown examples use disclosure buttons with explicit expanded state; a full menu needs its own keyboard model. Motion wrappers make exiting content inert.", performance: "These wrappers compose transform/opacity primitives. Drawer travel is relative to the surface size. Anchored surfaces need an explicit trigger-based transform origin." },
};
...
export const extendedDocs: AnimationDoc[] = [
  ...
  doc("checkbox-motion", "Feedback", "Draw the checkmark when a native checkbox becomes selected.", "line", [controlled("checked"), row("duration", "number (seconds)", "0.2", "Checkmark drawing duration; reduced motion is immediate.")]),
