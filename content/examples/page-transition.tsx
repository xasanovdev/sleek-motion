...
export default function Example({ reducedMotion = false, speed = 1 }: { reducedMotion?: boolean; speed?: number } = {}) {
  ...
  return <div><ToggleGroup value={[page]} onValueChange={(values) => { if (values.length) setPage(values[0]); }} aria-label="Preview pages" style={{ display: "flex", gap: 12 }}>{["Overview", "Activity"].map((name) => <Toggle key={name} value={name}>{name}</Toggle>)}</ToggleGroup>
