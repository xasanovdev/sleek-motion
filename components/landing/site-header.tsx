          ...
          <div className="flex items-center gap-8 text-xs/6">
            {copy.nav.map(([label, href]) => (
              <a
                key={href}
                href={href}
                className="rounded font-medium tracking-wide text-zinc-600 uppercase hover:text-brand-500"
              >
                {label}
              </a>
            ))}
...
function MobileNavigation({ copy }: { copy: LandingCopy }) {
  return (
    <div className="lg:hidden">
      <NavigationSheet
        label={copy.menu}
        title={copy.navAction}
        closeLabel={
          copy.localeHref === "/uz" ? "Close navigation" : "Menyuni yopish"
        }
        className="ui-icon-button ring-1 ring-zinc-950/10"
        trigger={<Bars3Icon className="size-5" aria-hidden="true" />}
      >
        {(close) => (
